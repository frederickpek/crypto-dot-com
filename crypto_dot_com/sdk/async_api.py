...
class CdcAsyncApi(CdcAsyncClient):
    ...
    async def get_user_balance_history(self, timeframe: str=None, end_time: int=None, limit: int=None) -> list[dict]:
        """
        ...
        if limit:
            params['limit'] = limit
        response = await self.request(method=POST, endpoint=USER_BALANCE_HISTORY, params=params)
    ...
    async def get_candlestick(self, instrument_name: str, timeframe: str=None, count: int=None, start_ts: int=None, end_ts: int=None) -> list[dict]:
        params = dict()
        params['instrument_name'] = instrument_name
        if timeframe:
            params['timeframe'] = timeframe
        ...
        response = await self.request(method=GET, endpoint=GET_CANDLESTICK, params=params)
        return response['result']['data']
